...
set PATH=%FORTIFY_HOME%\bin;%COVERITY_HOME%\bin;%JAVA_HOME%\bin;%ANT_HOME%\bin;%PATH%
...
set FORTIFY_BUILD_ID=myfortify
...
rmdir /q /s build
call cov-build --dir "%cov_tmp_dir%" ant -f build.xml
...
rem if not %errorlevel% == 0 goto FORTIFY_BUILD_ERROR
